...
public class AlgaeSubsystem extends SubsystemBase {
    ...
    private final SingleJointedArmSim pivotSim = new SingleJointedArmSim(DCMotor.getKrakenX60(2), 80,
            SingleJointedArmSim.estimateMOI(0.2, 3.0), Units.inchesToMeters(1.5),
            -110, 110,
            true, Angle);
    ...
    private final FlywheelSim algaeIntSim = new FlywheelSim(
            LinearSystemId.createFlywheelSystem(DCMotor.getNEO(1), 1.0, 4.0), DCMotor.getNEO(1));
    ...
    public double calculatevoltage() {
        if (Utils.isSimulation()) {
            return pivotFeedforward.calculate(Units.degreesToRadians(pivViz.getAngle()), 0)
                    + pivotController.calculate(pivViz.getAngle(), Angle);
        } else {
